...
var COORDS=[-1285, 19, 1317]

//potion eff vars

var POTIONEFFID=2
var POTIONEFFSTRENGTH=3
var POTIONEFFDURATION=20
var SHOULDPOTIONEFFBEVISIBLE=true
	...
	var closeentities=e.block.getWorld().getNearbyEntities(COORDS[0],COORDS[1],COORDS[2], DAMAGE_RADIUS, 1)
        if (closeentities.length!=0){
            for (var i = 0; i < closeentities.length; i++) {
                closeentities[i].addPotionEffect(POTIONEFFID,POTIONEFFDURATION,POTIONEFFSTRENGTH,SHOULDPOTIONEFFBEVISIBLE)
            }
        }
}
